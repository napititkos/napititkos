...
export function mergeProgress(local, server) {
  ...
  const tutorialDone = !!(server.tutorialDone || local.tutorialDone);
  const sharedResult = !!(server.sharedResult || local.sharedResult);
  ...
  return {
    ...
    tutorialDone,
    sharedResult,
    guestReplayDate: local.guestReplayDate || server.guestReplayDate || null,
  };
}

// Ha a mai titkosírást bejelentkezés NÉLKÜL (vendégként) fejtette meg valaki, majd
// bejelentkezik, a vendég-megfejtést visszavonjuk, hogy a fiókjával újra megfejthesse
// ugyanazon az eszközön. A sorozatot és a számlálókat a megfejtés előtti állapotra
// állítjuk vissza (a vendég-megfejtéskor elmentett pillanatkép alapján). Igazat ad
// vissza, ha volt mit visszavonni.
export function dropGuestEntry(prog, date) {
  const h = prog?.history?.[date];
  if (!h || !h.guest) return false;
  const u = h.undo || {};
  for (const k of ['streak', 'best', 'lastDate', 'totalSolved', 'noHintSolves', 'fastestTime']) {
    if (k in u) prog[k] = u[k];
  }
  delete prog.history[date];
  prog.guestReplayDate = date;
  return true;
}
